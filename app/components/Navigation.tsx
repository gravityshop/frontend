...
// NEU: Wir definieren die Props für unsere Nav-Schablone
...
const SECONDARY_LINKS = [{ label: "Log In", href: "/login" }];

// NEU: Destructuring der variant Prop (Standardwert ist "transparent")
    ...
    <>
      <header
        ...
        ${
          isMenuOpen
            ? "bg-transparent py-6"
            : isSolid // <--- HIER nutzen wir die neue Logik!
              ? "py-4 bg-[#050505]/80 backdrop-blur-xl border-b border-white/5 shadow-[0_4px_30px_rgba(0,0,0,0.3)]"
              : "py-8 bg-transparent"
        }`}
      >
          ...
          {MAIN_LINKS.slice(2).map((link) => (
            <div key={link.label} className="overflow-hidden">
              <a
                ...
                className="menu-link-item block font-['Anton'] text-2xl uppercase tracking-wider text-neutral-400 hover:text-white transition-colors duration-300"
              >
                {link.label}
              </a>
            </div>
